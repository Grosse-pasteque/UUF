from .constants import function, all_types
		...
		if isinstance(variables, list) and type(patterns) == list and isinstance(raiseerror, bool):
			if len(variables) == len(patterns):
				for key in range(len(variables)):
					...
					for i in all_types: pattern = pattern.replace(str(i), r'(.*){0,%d}' % maxseprange)
...
def Unpack(variable):
	if '{' in str(variable) or '}' in str(variable):
		raise KeyError(
			"dict aren't supported in this function !")
	
	return eval(str(variable).replace('[', '').replace('(', '').replace(']', '').replace(')', '').replace('"', '').replace("'", '').split(', '))
